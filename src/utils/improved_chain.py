...
from .parse_entities import parse_entities_flexible
    ...
    def prepare_entity_chain(self):
        entity_chain_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "Extract all important entities (like person names, movie names, or years) from the user's question. "
        ...
        entity_chain = entity_chain_prompt | self.llm | StrOutputParser() | RunnableLambda(parse_entities_flexible)
        return entity_chain
    ...
    def prepare_cypher_response(self, entity_chain, cypher_prompt):
        ...
        cypher_response = (
            ...
            | RunnablePassthrough.assign(
                entities_list=lambda x: self.map_to_database(x),
